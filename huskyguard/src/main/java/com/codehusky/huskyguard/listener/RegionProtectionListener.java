...
import com.codehusky.huskyguard.protection.events.DisallowedPVPEvent;
import com.codehusky.huskyguard.util.Entities;
import com.codehusky.huskyguard.util.Events;
import com.codehusky.huskyguard.util.InteropUtils;
...
public class RegionProtectionListener extends AbstractListener {
     ...
     */
    public RegionProtectionListener(HuskyGuardPlugin plugin) {
        super(plugin);
    }
    ...
    private void tellErrorMessage(DelegateEvent event, Cause cause, Location location, String what) {
        ...
        if (rootCause instanceof Player) {
            ...
            Long lastTime = HGMetadata.getIfPresent(player, DENY_MESSAGE_KEY, Long.class);
            if (lastTime == null || now - lastTime >= LAST_MESSAGE_DELAY) {
                ...
                HGMetadata.put(player, DENY_MESSAGE_KEY, now);
            }
        }
    }
    ...
    public void onDamageEntity(DamageEntityEvent event) {
        ...
        /* PVP */
            ...
            LocalPlayer localAttacker = HuskyGuardPlugin.inst().wrapPlayer(playerAttacker);
        ...
        if (vehicle instanceof Tameable && exited instanceof Player) {
            ...
            if (!isWhitelisted(Cause.create(player), vehicle.getWorld(), false)) {
                ...
                if (!query.testBuild(BukkitAdapter.adapt(location), localPlayer, Flags.RIDE, Flags.INTERACT)) {
                    long now = System.currentTimeMillis();
                    Long lastTime = HGMetadata.getIfPresent(player, DISEMBARK_MESSAGE_KEY, Long.class);
                    if (lastTime == null || now - lastTime >= LAST_MESSAGE_DELAY) {
                        player.sendMessage("" + ChatColor.GOLD + "Don't disembark here!" + ChatColor.GRAY + " You can't get back on.");
                        HGMetadata.put(player, DISEMBARK_MESSAGE_KEY, now);
                    }
