...
public class ChestProtectionListener extends AbstractListener {
     ...
     */
    public ChestProtectionListener(HuskyGuardPlugin plugin) {
        super(plugin);
    }
    ...
    public void onPlaceBlock(final PlaceBlockEvent event) {
        ...
        if (player != null) {
            final SpongeWorldConfiguration wcfg = (SpongeWorldConfiguration) getWorldConfig(HuskyGuardPlugin.inst().wrapPlayer(player));
            ...
            event.filter(target -> {
                if (wcfg.getChestProtection().isChest(BukkitAdapter.asBlockType(event.getEffectiveMaterial())) && wcfg.isChestProtected(BukkitAdapter.adapt(target.getBlock().getLocation()),
                        HuskyGuardPlugin.inst().wrapPlayer(player))) {
    ...
    public void onBreakBlock(final BreakBlockEvent event) {
        ...
        final SpongeWorldConfiguration wcfg = (SpongeWorldConfiguration) getWorldConfig(BukkitAdapter.adapt(event.getWorld()));
        ...
        if (player != null) {
            final LocalPlayer localPlayer = HuskyGuardPlugin.inst().wrapPlayer(player);
            event.filter(target -> {
    ...
    public void onSignChange(SignChangeEvent event) {
        ...
        SpongeWorldConfiguration wcfg = (SpongeWorldConfiguration) getWorldConfig(HuskyGuardPlugin.inst().wrapPlayer(player));

        if (wcfg.signChestProtection) {
            if ("[Lock]".equalsIgnoreCase(event.getLine(0))) {
                if (wcfg.isChestProtectedPlacement(BukkitAdapter.adapt(event.getBlock().getLocation()), HuskyGuardPlugin.inst().wrapPlayer(player))) {
